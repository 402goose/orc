...
function focusTask(focus, report) {
  const task = String(report?.task || focus.task || "");
  const packet = report?.admission?.request?.intent?.label_draft;
  const decision = packet?.schema === "fusion.label-draft-packet.v1" && typeof packet.decision?.id === "string" ? packet.decision : null;
  const firstLine = task.trim().split(/\r?\n/)[0] || "Untitled task";
  const title = decision ? "Draft labels for human review" : firstLine.length > 180 ? firstLine.slice(0, 177).trimEnd() + "…" : firstLine;
  const source = decision ? `<p class="room-task-source">Source ${esc(decision.kind || "")} decision · <a href="#decisions/review?${esc(new URLSearchParams({w:state.workspace, decision:decision.id}).toString())}">${esc(decision.id)}</a></p>` : "";
  return `<h2>${esc(title)}</h2>${source}${task !== title ? `<details class="room-task" data-disclosure-key="full-task-${esc(focus.id)}"><summary>Full task · ${number(task.length)} characters</summary><pre tabindex="0">${esc(task)}</pre></details>` : ""}`;
}
function productProgress() {
  const value = state.activity?.product_progress;
  if (!value) return "";
  if (value.status !== "ready") return `<section class="panel"><div class="panel-body"><h2>Product progress unavailable</h2><p class="room-evidence-missing">${esc(value.error || "Checkpoint could not be read")}</p></div></section>`;
  return `<section class="product-progress" aria-label="Product progress"><div class="section-bar"><h2>What we are building</h2><small>Coordinator checkpoint · ${date(Date.parse(value.updated_at))}</small></div><div class="product-progress-grid">${value.projects.map(project => `<article class="panel product-progress-card"><div class="panel-body"><div class="eyebrow">${esc(project.phase)}</div><h3>${esc(project.name)}</h3><dl>${[["Working",project.working],["Current work",project.current],["Next outcome",project.next],["Still unqualified",project.limits]].map(([label,text]) => `<dt>${label}</dt><dd>${esc(text)}</dd>`).join("")}</dl><div class="actions">${(project.evidence || []).map(ref => `<a class="button small ghost" href="#workflow/${encodeURIComponent(ref.run_id)}?w=${encodeURIComponent(ref.workspace_id)}" data-action="workspace-run" data-workspace="${esc(ref.workspace_id)}" data-id="${esc(ref.run_id)}">${esc(ref.label)} →</a>`).join("")}</div></div></article>`).join("")}</div><p class="help-copy">${esc(value.note)}</p>${value.milestones?.length ? `<details class="product-roadmap" data-disclosure-key="product-roadmap"><summary>Milestones · ${value.milestones.filter(m=>m.status === "qualified").length} / ${value.milestones.length} qualified</summary><ol>${value.milestones.map(m=>`<li><div><strong>${esc(m.name)}</strong><span class="pill">${esc(m.status.replaceAll("_", " "))}</span></div><p>${esc(m.exit)}</p></li>`).join("")}</ol><small>Qualification is recorded by the coordinator against the stated exit conditions. Run evidence is linked above.</small></details>` : ""}</section>`;
}
...
function gardenPanel() {
  ...
  return `<section class="panel garden-panel"><div class="garden-main"><div class="eyebrow guild-section-emblem">${sigil("seed")} DATA GARDEN · ${g.max_drafts_per_day ? esc(g.max_drafts_per_day) + " DRAFTS / UTC DAY" : "NO DAILY CAP"}</div><h2>${title}</h2><p>${ready ? (automatic ? "Supported council answers are approved automatically. These remaining drafts need attention: inspect their votes, unavailable workers and evidence." : "Open Review to inspect the pending labels and evidence. Drafts require approval before they enter training exports.") : automatic ? "The council assesses new decisions and approves unanimous answers automatically. Disagreements stay in your review queue." : "Automatically draft labels as decisions arrive. You edit and approve the useful ones."}</p>
...
document.addEventListener("click", async (event) => {
  ...
  if (!target) return;
  if (target.matches('a[data-action="workspace-run"]')) {
    // Preserve native Cmd/Ctrl-click, new-window and middle-click navigation.
