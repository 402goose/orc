...
function productProgress() {
  ...
  return `<section class="product-progress" aria-label="Product progress"><div class="section-bar"><h2>What we are building</h2><small>Coordinator checkpoint · ${date(Date.parse(value.updated_at))}</small></div><div class="product-progress-grid">${value.projects.map(project => `<article class="panel product-progress-card"><div class="panel-body"><div class="eyebrow">${esc(project.phase)}</div><h3>${esc(project.name)}</h3><dl>${[["Working",project.working],["Current work",project.current],["Next outcome",project.next],["Still unqualified",project.limits]].map(([label,text]) => `<dt>${label}</dt><dd>${esc(text)}</dd>`).join("")}</dl><div class="actions">${(project.evidence || []).map(ref => button(esc(ref.label) + " →", "workspace-run", `data-workspace="${esc(ref.workspace_id)}" data-id="${esc(ref.run_id)}"`, "small ghost")).join("")}</div></div></article>`).join("")}</div><p class="help-copy">${esc(value.note)}</p>${value.milestones?.length ? `<details class="product-roadmap" data-disclosure-key="product-roadmap"><summary>Milestones · ${value.milestones.filter(m=>m.status === "qualified").length} / ${value.milestones.length} qualified</summary><ol>${value.milestones.map(m=>`<li><div><strong>${esc(m.name)}</strong><span class="pill">${esc(m.status.replaceAll("_", " "))}</span></div><p>${esc(m.exit)}</p></li>`).join("")}</ol><small>Qualification is recorded by the coordinator against the stated exit conditions. Run evidence is linked above.</small></details>` : ""}</section>`;
}
